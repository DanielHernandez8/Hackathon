...
@Component // Para que se ejecute la clase cuando se arranqueu la aplicaci�n
@Slf4j // Lombok log para poder escribir comentarios de forma m�s sencilla
public class Runner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		if (this.userRepository.count() == 0) {
			...
			// User: dani, Pass: 1234, Authority: READ
			this.userRepository.save(new User("dani", new BCryptPasswordEncoder().encode("1234"),
					(List<Authority>) List.of(this.authorityRepository.findByName(AuthorityName.READ).get())));
			// User: hernandez, Pass: 1234, Authority: WRITE
			this.userRepository.save(new User("hernandez", new BCryptPasswordEncoder().encode("1234"),
					(List<Authority>) List.of(this.authorityRepository.findByName(AuthorityName.WRITE).get())));
			// User: montero, Pass: 1234, Authority: ADMIN
			this.userRepository.save(new User("montero", new BCryptPasswordEncoder().encode("1234"),
